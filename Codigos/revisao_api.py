...
byd = df.loc[filtro]
byd_codigo = byd["valor"].iloc[0]   # pega o código como string
codigoMarca = byd_codigo
...
# 3 - O Banco Mundial disponibiliza uma API pública - https://api.worldbank.org/v2/country/{pais}/indicator/{indicador}?format=json
# com diversos indicadores econômicos. 
...
api = f"https://api.worldbank.org/v2/country/{pais}/indicator/{indicador}?format=json"
dados = requests.get(api).json()[1]
df = pd.DataFrame(dados)

df = df[["date", "value"]]
min = df["value"].min()
filtro = (df["value"] == min)
df = df.loc[filtro,"date"]
print(min)
...
dados = requests.get(api).json()
df = pd.DataFrame(dados)
filtro = df.loc(["value" == "licenciamento"])

dados = requests.get(api).json()["value"]

df = pd.DataFrame(dados)
df_anfavea = df[df["FNTSIGLA"].str.contains("ANFAVEA", case=False)]
...
df_anfavea["SERCOMENTARIO"]

CODIGO_ENCONTRADO = "ANFAVE12_LICVETOT12"
...
# 5 - Utilize a API PTAX do Banco Central (endpoint CotacaoDolarPeriodo) para obter as 
# cotações do dólar (compra e venda) em um período definido por você (ex.: de 01/01/2023 a 31/12/2023).
...
url = (
    ...
    "@dataInicial='01-01-2024'&@dataFinalCotacao='31-12-2024'&$format=json"
)
